...
function getComment(api) {
    ...
    const defaultEmptyString =
        api.field_type_text.join() === 'String' &&
        !defaultValue &&
        !api.field_enum;
...
function getPropsApiType(api, framework) {
    ...
    if (hasTNodes) {
        ...
        if (csType) {
            ...
            type = baseType
                .map((item) => formatOneType(api, item, framework))
                .join(' | ');
        }
    }
...
function replaceInputEvent(str, newApi) {
    ...
    const reg = new RegExp(
        `(${REACT_EVENTS.concat('ImageEvent').join('|')})`,
        'g'
    );
    ...
    newStr = newStr.replace(reg, (val) => {
        ...
        return (
            r ||
            `${val}<${
                ['FormEvent', 'InputEvent'].includes(val)
                    ? 'HTMLInputElement'
                    : 'HTMLDivElement'
            }>`
        );
...
function handleApiByFramework(api, framework) {
    ...
    if (framework.indexOf('Vue') !== -1 && newApi.custom_field_type) {
        ...
        const filters = VUE_IGNORE_TYPES.filter(
            (item) => newApi.custom_field_type.indexOf(item) !== -1
        );
        if (filters.length) {
...
function getTypeScriptDesc(componentMap, framework) {
    ...
    Object.keys(componentMap).forEach((cmp) => {
        ...
        apis.forEach((apiData) => {
            ...
            if (
                ['Return', 'Extends', '<T>'].includes(api.field_category_text)
            ) {
                ...
                if (r.desc instanceof Array && r.desc.length) {
                    ...
                    if (type) {
                        // API 是否为组件实例方法
                        const isInstanceFuncs = isComponentFunction(
                            cmp,
                            api.field_category_text
                        );
                        if (isInstanceFuncs) {
        ...
        if (isPluginFunctions) {
            exports = exports.concat(
                `export type ${getPluginName(cmp)} = (${body.join(', ')}) => ${
                    returnName || 'void'
                };`
            );
            body = [];
    ...
    Object.keys(rMap).forEach((cmp) => {
        ...
        list.forEach((innerCmp) => {
            ...
            Object.keys(current).forEach((type) => {
                ...
                if (ts[innerCmp]) {
                    current[type] = [
                        ...new Set(current[type].concat(ts[innerCmp][type])),
                    ];
                }
            });
        });
    ...
    Object.keys(ts).forEach((cmp) => {
        ...
        if (
            ...
            ts[cmp].imports = ts[cmp].imports.concat(
                getGlobalsImports(bodyDesc + exportDesc, framework)
            );
        }
        const r = [
