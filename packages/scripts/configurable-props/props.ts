...
import { isPlugin, getCmpTypeCombineMap, getFolderName } from '../common.ts';
...
function getOneApiInfo(api: ApiData, category: string) {
    ...
    Object.keys(fieldCategoryMap).forEach((category) => {
      ...
      apis.forEach((api) => {
        ...
        if (
          ...
          oneApiInfo && (info[p] as unknown[]).push(oneApiInfo);
        }
      });
    });
    result[cmp] = info;
...
function generateProps(baseData, framework) {
  ...
  const apiInfos = getApiInfos(baseData);
