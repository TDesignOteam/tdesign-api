...
export interface TdUploadProps {
   ...
   */
  onAdd?: (files: MediaContext) => void;
  /**
   * 点击已选文件时触发；常用于重新上传
   */
  onClick?: (index: number, file: VideoContext | ImageContext) => void;
  /**
   * 上传成功或失败后触发
   */
  onComplete?: () => void;
   ...
   */
  onDrop?: (files: MediaContext) => void;
  /**
   * 上传失败后触发
   */
  onFail?: () => void;
