...
import { generateTypes } from './types/index.ts';
import { generateReactDefaultProps } from './types/react-default-props.ts';
import { generateVueProps } from './types/vue-props.ts';
import { generateUnitTest } from './unit/index.ts';
import { generateVitestUnitCase } from './vitest/generateVitestUnitCase.ts';
import { generateTestDescriptionToVitestFile } from './vitest/tests/core/utils.ts';
...
async function generateComponentApi() {
  ...
  if (!onlyDocs) {
    ...
    // 生成 props 单元测试文件
    if (isUseUnitTest) {
      generateUnitTest(baseData, framework, { component: currentComponent });
    }
  }
  // 生成 API 中文文档
