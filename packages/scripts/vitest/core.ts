...
import { UNIT_TEST_EVENTS_MAP } from './const/events-map.ts';
import { reactNeedMockDelayEvents } from './const/react-need-mock-delay.ts';
import type { ApiData, WrapperExtraData, FireEventParams, EventArgumentsExtra, EventArgExpect, DomTextExpect, DomAttributeExpect } from '../shared-types.ts';
...
function getItDescription(oneApiData: ApiData) {
  const type = oneApiData.field_category_text.toLocaleLowerCase();
  return `'${type}.${oneApiData.field_name} works fine'`;
}

function getFullMountCode(framework: string, componentCode: string): string | undefined {
  if (framework === 'Vue(PC)') {
    return `mount({
        render() {
          return (
 ...
 */
function getMountComponent(framework: string, component: string, props: Record<string, unknown>, extra: { content?: string; wrapper?: string } = {}): string | undefined {
  ...
  if (wrapper) {
    ...
    const properties = props
      ? Object.keys(props)
          ...
          .join(' ')
      : '';
    const eventsCode = events && typeof events === 'string' && !events.includes('on=') && framework === 'Vue(PC)' ? `on={${events}}` : events as string;
...
function getEventFunctions(expect: EventArgExpect[], framework: string, extraCode: { wrapper?: string }) {
 ...
 * @returns 快照代码
 */
function getSnapshotCase(snapshot: boolean, framework: string, wrapperIndex = '', onlyDocumentDom?: boolean) {
  if (!snapshot) return;
  if (onlyDocumentDom) {
 ...
 * @returns
 */
function getWrapper(framework: string, mountCode: string, goalDom: string | undefined = '', wrapperIndex = '', extraData: WrapperExtraData = {}) {
 ...
 * @returns
 */
function getDomCountExpectCode(framework: string, domAndCount: Record<string, unknown>, wrapperIndex = '', eventIndex = '') {
  let clearElement = '';
  const arr = Object.entries(domAndCount).map(([className, countOrIndex]) => {
    // 如果是清空
    if (className === 'clearElementAtEnd') {
      clearElement = countOrIndex as string;
      return;
    }
    if (typeof countOrIndex === 'object' && countOrIndex !== null) {
      const objVal = countOrIndex as Record<string, unknown>;
      if (objVal.className) {
        // 已在内部处理 document selector
        return getOneDomClassNameExpectCode(framework, className, objVal, wrapperIndex, eventIndex);
      }
    }
    if (typeof countOrIndex === 'number' || countOrIndex === false) {
      return getOneDomCountExpectCode(framework, className, countOrIndex as number | false, wrapperIndex);
    }
    if (typeof countOrIndex === 'object' && countOrIndex !== null) {
      const objVal = countOrIndex as Record<string, unknown>;
      // textContent
      if (objVal.text) {
        return getOneDomTextExpectCode(framework, className, objVal as DomTextExpect, wrapperIndex);
      }
      if (objVal.attribute) {
        return getOneDomAttributeExpectCode(framework, className, objVal as DomAttributeExpect, wrapperIndex, eventIndex);
      }
    }
  });
  if (clearElement) {
...
function getClassNameExpectTruthy(framework: string, className: string, wrapperIndex = '', goalDom = '') {
  if (framework.indexOf('Vue') !== -1) {
...
function getClassNameExpectFalsy(framework: string, className: string, wrapperIndex = '', goalDom = '') {
  if (framework.indexOf('Vue') !== -1) {
...
function getVueOneAttributeCode(framework: string, wrapper: string, attribute: string, value: unknown, extra: { isDocumentNode?: boolean } = {}) {
...
function makeObjectToString(objectArgs: Record<string, unknown>) {
  const arr = ['{'];
  const argsList = Object.entries(objectArgs)
    .map(([key, value]) => {
      ...
      if (typeof tmpValue === 'object' && tmpValue !== null) {
        tmpValue = makeObjectToString(tmpValue as Record<string, unknown>);
      }
      return `'${key}': ${tmpValue}`;
    })
    .join(',');
...
// 事件参数测试
function getEventArguments(framework: string, args: unknown, extra: EventArgumentsExtra = {}) {
 ...
 */
function getFireEventCode(framework: string, { dom, event, delay, component }: FireEventParams, wrapperIndex = '', eventIndex = '') {
  let fireEventCode = [];
  if (event !== 'delay') {
  ...
  let delayTime: string | number = (delay && delay !== true) || delay === 0 ? (delay as string | number) : '';
  if (framework.indexOf('Vue') !== -1) {
    if (!delayTime && event === 'delay' && !isNaN(Number(dom))) {
      delayTime = dom;
    }
    ...
    if (getReactNeedMockDelay(event, delay) || event === 'delay') {
      if (event === 'delay' && !isNaN(Number(dom))) {
        delayTime = dom;
      }
      fireEventCode.push(`await mockDelay(${delayTime || ''});`);
    }
...
function getPresetsExpect(triggerList: string | string[], framework: string, component: string) {
...
function getItAsync(trigger: string | string[] | undefined, framework: string) {
  const { reactAsync } = getReactFireEventAsync(getTriggerList(trigger), framework);
  return (framework.indexOf('Vue') !== -1 && trigger) || reactAsync || (typeof trigger === 'string' && trigger.includes('delay')) || (Array.isArray(trigger) && trigger.some(t => t.includes('delay'))) ? 'async' : '';
}

function getCategoryDesc(oneApiData: ApiData, component: string) {
