 ...
 */
import { SIMULATE_FUNCTIONS, GET_VAR_FUNCTIONS } from './core.ts';
import type { ImportsConfigParams, TNodeConfig, MoreEventImportsResult } from '../shared-types.ts';
...
function getImportsConfig(params: ImportsConfigParams = {}, tests: string[]) {
  const {
    hasEvent = false,
    importedComponents = [],
...
function getImportsCode(importsConfig: Record<string, Record<string, string | string[]>>, framework: string, customImported: string[] = []) {
...
function getMoreEventImports(framework: string, event: unknown, wrapper?: string, trigger?: string | string[]): MoreEventImportsResult {
  const importedTestUtils: string[] = [];
  const importedMounts: string[] = [];
  if (!Array.isArray(event)) return { importedTestUtils, importedMounts };
...
// 引入公共函数/变量
function getVariableImports(test: { variables?: string[]; tnode?: TNodeConfig | TNodeConfig[] }) {
  const { variables, tnode } = test;
  const allVariables: string[] = variables || [];
