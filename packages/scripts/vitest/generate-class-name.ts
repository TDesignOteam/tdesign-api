...
import { getSkipCode } from './utils.ts';
import type { ApiData, TestDescription, ClassNameArrayItem } from '../shared-types.ts';

/**
 * 输出类名检测单测用例
 */
function generateClassNameUnitCase(test: TestDescription, oneApiData: ApiData, framework: string, component: string) {
  const arr = generateVueAndReactClassName(test, oneApiData, framework, component);
  return arr && arr.filter((v) => v);
}

function generateVueAndReactClassName(test: TestDescription, oneApiData: ApiData, framework: string, component: string) {
  ...
  const mountCode = getMountComponent(
    framework,
    component,
    { ...props, [oneApiData.field_name]: '/-item-/' },
    extraCode,
  );
  ...
  if (
    ...
    enums.length &&
    oneApiData.field_type_text[0] === 'String'
  ) {
    const arr = [
  ...
  if (Array.isArray(className)) {
    ...
    if (oneApiData.field_type_text[0] === 'String' && enums.length) {
      ...
      const propsCode = getCategoryDesc(oneApiData, component);
      return (className as ClassNameArrayItem[]).map(({ value, expect }) => {
        const mountCode = getMountComponent(
          framework,
          component,
          { ...props, [oneApiData.field_name]: value },
          extraCode,
        );
        const arr = [
  ...
  if (typeof className === 'string') {
    if (oneApiData.field_type_text[0] === 'Boolean') {
      ...
      const mountCode2 = getMountComponent(
        framework,
        component,
        { ...props, [oneApiData.field_name]: true },
        extraCode,
      );
      const mountCode3 = getMountComponent(
        framework,
        component,
        { ...props, [oneApiData.field_name]: false },
        extraCode,
      );
      const arr = [
  ...
  if (typeof className === 'object' && !Array.isArray(className)) {
    ...
    const mountCode = getMountComponent(
      framework,
      component,
      { ...props, [oneApiData.field_name]: '/-propValue-/' },
      extraCode,
    );
    const arr = [
...
export { generateClassNameUnitCase };
