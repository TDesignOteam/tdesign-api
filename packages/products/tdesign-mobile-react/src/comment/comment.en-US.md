...
fold | Object | - | Typescript: `CommentFold ` `type CommentFoldState = 'collapsed' \| 'expanded' \| 'all'` `interface CommentFold { state: CommentFoldState; total?: number; visible?: number; step?: number; text?: Partial<Record<CommentFoldState, string>>;}`。[see more ts definition](https://github.com/Tencent/tdesign-mobile-react/tree/develop/src/comment/type.ts) | N
