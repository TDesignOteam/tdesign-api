...
fold | Object | - | 折叠/展开配置项。支持语法糖 `v-model` 或 `v-model:fold`。TS 类型：`CommentFold ` `type CommentFoldState = 'collapsed' \| 'expanded' \| 'all'` `interface CommentFold { state: CommentFoldState; total?: number; visible?: number; step?: number; text?: Partial<Record<CommentFoldState, string>>;}`。[详细类型定义](https://github.com/Tencent/tdesign-mobile-vue/tree/develop/src/comment/type.ts) | N
...
fold | `(context: { fold: CommentFold; e: Event })` | 用户点击“展开回复/收起”时触发，并返回点击后的 fold 配置
