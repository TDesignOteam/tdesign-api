...
onClose | Function |  | TS 类型：`(context: { trigger: 'overlay' \| 'close-button', visible: Boolean, index: Number }) => void`<br/>关闭时触发 | N
...
名称 | 参数 | 描述
-- | -- | --
close | `(context: { trigger: 'overlay' \| 'close-button', visible: Boolean, index: Number })` | 关闭时触发
