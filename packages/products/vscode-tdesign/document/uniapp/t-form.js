...
statusIcon | Boolean / TNode | undefined | 校验状态图标，值为 \`true\` 显示默认图标，默认图标有 成功、失败、警告 等，不同的状态图标不同。\`statusIcon\` 值为 \`false\`，不显示图标。\`statusIcon\` 值类型为渲染函数，则可以自定义右侧状态图标。TS 类型：\`boolean \| TNode<TdFormItemProps>\`。[通用类型定义](https://github.com/novlan1/tdesign-uniapp/blob/develop/packages/tdesign/common/common.ts) | N
submitWithWarningMessage | Boolean | false | 【讨论中】当校验结果只有告警信息时，是否触发 \`submit\` 提交事件 | N
supportNumberKey | Boolean | true | 是否支持使用数字作为表单键值，在1.9.3版本后表单组件支持数字作为键值，若仍需要保留数字作为数组下标，请关闭此API | N
onReset | Function |  | TS 类型：\`(context: { e?: FormResetEvent }) => void\`<br/>表单重置时触发。[通用类型定义](https://github.com/novlan1/tdesign-uniapp/blob/develop/packages/tdesign/common/common.ts) | N
...
onValidate | Function |  | TS 类型：\`(result: ValidateResultContext<FormData>) => void\`<br/>校验结束后触发，result 值为 true 表示校验通过；如果校验不通过，result 值为校验结果列表。[详细类型定义](https://github.com/novlan1/tdesign-uniapp/blob/develop/packages/tdesign/form/type.ts)。<br/>\`type ValidateResultContext<T extends Data> = Omit<SubmitContext<T>, 'e'>\`<br/> | N
...
values-change | \`(changedValues: Record<string, unknown>, allValues: Record<string, unknown>)\` | 字段值更新时触发的回调事件
...
getFieldsValue | \`(nameList: string[] \| boolean)\` | \`getFieldsValue<FormData>\` | 必需。获取一组字段名对应的值，当调用 getFieldsValue(true) 时返回所有表单数据。[详细类型定义](https://github.com/novlan1/tdesign-uniapp/blob/develop/packages/tdesign/form/type.ts)。<br/>\`interface getFieldsValue<T>{ (nameList: true): T; (nameList: any[]): Record<keyof T, unknown>;}\`<br/>
...
setFields | \`(fields: FieldData[])\` | \- | 必需。设置多组字段状态。TS 类型：\`(fields: FieldData[]) => void\` \`interface FieldData { name: NamePath; value?: unknown, status?: string, validateMessage?: { type?: string, message?: string } }\`。[详细类型定义](https://github.com/novlan1/tdesign-uniapp/blob/develop/packages/tdesign/form/type.ts)
setFieldsValue | \`(field: Data)\` | \- | 必需。设置表单字段值
setValidateMessage | \`(message: FormValidateMessage<FormData>)\` | \- | 必需。设置自定义校验结果，如远程校验信息直接呈现。注意需要在组件挂载结束后使用该方法。\`FormData\` 指表单数据泛型。[详细类型定义](https://github.com/novlan1/tdesign-uniapp/blob/develop/packages/tdesign/form/type.ts)。<br/>\`type FormValidateMessage<FormData> = { [field in keyof FormData]: FormItemValidateMessage[] }\`<br/><br/>\`interface FormItemValidateMessage { type: 'warning' \| 'error'; message: string }\`<br/>
...
help | String / TNode | - | 表单项说明内容。TS 类型：\`string \| TNode\`。[通用类型定义](https://github.com/novlan1/tdesign-uniapp/blob/develop/packages/tdesign/common/common.ts) | N
initialData | String / Number / Object / Array | - | 表单初始数据，重置时所需初始数据。TS 类型：\`InitialData\` \`type InitialData = any\`。[详细类型定义](https://github.com/novlan1/tdesign-uniapp/blob/develop/packages/tdesign/form/type.ts) | N
label | String / TNode | '' | 字段标签名称。TS 类型：\`string \| TNode\`。[通用类型定义](https://github.com/novlan1/tdesign-uniapp/blob/develop/packages/tdesign/common/common.ts) | N
...
name | String / Number / Array | - | 表单字段名称。TS 类型：\`NamePath\` \`type NamePath = string \| number \| Array<string \| number>\`。[详细类型定义](https://github.com/novlan1/tdesign-uniapp/blob/develop/packages/tdesign/form/type.ts) | N
...
statusIcon | Boolean / TNode | undefined | 校验状态图标，值为 \`true\` 显示默认图标，默认图标有 成功、失败、警告 等，不同的状态图标不同。\`statusIcon\` 值为 \`false\`，不显示图标。\`statusIcon\` 值类型为渲染函数，则可以自定义右侧状态图标。优先级高级 Form 的 statusIcon。TS 类型：\`boolean \| TNode\`。[通用类型定义](https://github.com/novlan1/tdesign-uniapp/blob/develop/packages/tdesign/common/common.ts) | N
successBorder | Boolean | false | 是否显示校验成功的边框，默认不显示 | N
tips | String / TNode | - | 自定义提示内容，样式跟随 \`status\` 变动，可在需要完全自主控制校验规则时使用。TS 类型：\`string \| TNode\`。[通用类型定义](https://github.com/novlan1/tdesign-uniapp/blob/develop/packages/tdesign/common/common.ts) | N
valueFormat | Function | - | 当用户交互产生数据变化时触发，用于格式化数据。TS 类型：\`FormItemFormatType\` \`type FormItemFormatType = (value: any) => any\`。[详细类型定义](https://github.com/novlan1/tdesign-uniapp/blob/develop/packages/tdesign/form/type.ts) | N
