...
onColumnResizeChange | Function |  | TS 类型：`(context: { column: PrimaryTableCol<T>; index: number; }) => void`<br/>列调整大小之后触发。`context.column` 表示操作的列；`context.index` 表示操作的列的序号； | N
