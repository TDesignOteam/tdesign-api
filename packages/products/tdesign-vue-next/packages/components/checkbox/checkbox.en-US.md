...
tooltipProps | \- | - | Transparent all feature props of the Tooltip. Used to customize the hover popup content and style, applied to the checkbox input box only。Typescript: `TooltipProps`，[Tooltip API Documents](./tooltip?tab=api)。[see more ts definition](https://github.com/Tencent/tdesign-vue-next/blob/develop/packages/components/checkbox/type.ts) | N
