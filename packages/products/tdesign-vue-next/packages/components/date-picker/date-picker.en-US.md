...
format | String | 'YYYY-MM-DD' | The `format` parameter is used solely for formatting the date display and does not affect the actual date value. Note the distinction from `valueType`: `valueType` directly determines the data format of the date value `value`. The global default configuration is `'YYYY-MM-DD'`. [Refer to the Day.js formatting documentation for details](https://day.js.org/docs/en/display/format) | N
