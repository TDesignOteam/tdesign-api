...
splitStyle | Object | - | Customize the inline style of the splitter。Typescript: `Styles`。[see more ts definition](https://github.com/Tencent/tdesign-vue-next/blob/develop/packages/components/common.ts) | N
...
-- | -- | --
resize | `(sizes: number[])` | Callback (array of pixel values) for size changes during dragging.
resize-end | `(sizes: number[])` | Callback when drag ends
resize-start | `(sizes: number[])` | Callback when drag starts
